//! Checked-in JSON Schema drift gate for `cauterizer-api`'s published contracts.
//!
//! Mirrors `crates/contexts/organization-access/tests/schema_drift.rs` exactly
//! (ADR-021's reusable pattern, not a new one): regenerates the JSON Schema for
//! every request/response type this crate's handlers actually accept or return
//! from the live Rust type via `schemars`, and compares it byte-for-byte against
//! the checked-in file under `schemas/cauterizer-api/`.
//!
//! Regenerate intentionally with:
//! `CAUTERIZER_UPDATE_SCHEMA=1 cargo test -p cauterizer-api --test schema_drift`

use cauterizer_api::contracts::{BootstrapOrganizationRequestV1, BootstrapOrganizationResponseV1};
...
const CONTEXT: &str = "cauterizer-api";
...
fn assert_schema_matches<T: JsonSchema>(name: &str) {
    ...
    let Ok(expected) = fs::read_to_string(&snapshot) else {
        panic!(
            "missing schema snapshot {} for {CONTEXT}/{name}; regenerate with `{UPDATE_ENV}=1 cargo test -p cauterizer-api --test schema_drift`",
            snapshot.display()
        )
    };
    if expected == actual {
    ...
    panic!(
        "checked-in schema for {CONTEXT}/{name} drifted from the live Rust type ({guidance}); \
         ...
         `{UPDATE_ENV}=1 cargo test -p cauterizer-api --test schema_drift`"
    );
}
...
fn workspace_root() -> &'static Path {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("cauterizer-api must live below <workspace>/crates")
}

#[test]
fn bootstrap_organization_request_schema_has_not_drifted() {
    assert_schema_matches::<BootstrapOrganizationRequestV1>("bootstrap-organization-request.v1");
}

#[test]
fn bootstrap_organization_response_schema_has_not_drifted() {
    assert_schema_matches::<BootstrapOrganizationResponseV1>("bootstrap-organization-response.v1");
}
