//! Checked-in JSON Schema drift gate for Advisory Intake's published contracts.
//!
...
//! `CAUTERIZER_UPDATE_SCHEMA=1 cargo test -p cauterizer-advisory-intake --test schema_drift`

use cauterizer_advisory_intake::contracts::{AdvisoryIntakeEventV1, AdvisorySnapshotV1};
...
const CONTEXT: &str = "advisory-intake";
...
fn assert_schema_matches<T: JsonSchema>(name: &str) {
    ...
    let Ok(expected) = fs::read_to_string(&snapshot) else {
        panic!(
            "missing schema snapshot {} for {CONTEXT}/{name}; regenerate with `{UPDATE_ENV}=1 cargo test -p cauterizer-advisory-intake --test schema_drift`",
            snapshot.display()
        )
    };
    if expected == actual {
    ...
    panic!(
        "checked-in schema for {CONTEXT}/{name} drifted from the live Rust type ({guidance}); \
         ...
         `{UPDATE_ENV}=1 cargo test -p cauterizer-advisory-intake --test schema_drift`"
    );
}
...
fn workspace_root() -> &'static Path {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .and_then(Path::parent)
        .expect("cauterizer-advisory-intake must live below <workspace>/crates/contexts")
}

#[test]
fn advisory_snapshot_schema_has_not_drifted() {
    assert_schema_matches::<AdvisorySnapshotV1>("advisory-snapshot.v1");
}

#[test]
fn advisory_intake_event_schema_has_not_drifted() {
    assert_schema_matches::<AdvisoryIntakeEventV1>("advisory-intake-event.v1");
}
